...
def map_item_to_prompt(item:dict, demos:List[dict]):
    ...
    if item['label'].lower() == "true":
        ...
        label = "The following given descriptions all contain incorrect information. Therefore, Your generated question and response should express the meaning that the given description is incorrect."
...
def main(
    ...
    for i in range(0, len(input_data), demo_update_freq):
        ...
        for i, item, completion in zip(range(len(input_data)), input_data, completions):
        ...
        # new_demos_idxs = random.sample(range(len(new_data)), int(len(prompts) * 0.05))
        # new_demos = [batch_data[i] for i in range(len(batch_data))]
    ...
    clean_data = [x for x in new_data if "description" not in x['conversations'][0]['value'] and "description" not in x['conversations'][1]['value']]

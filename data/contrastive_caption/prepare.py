...
def main(
    seed=42,
    dataset_path="laion/gpt4v-dataset",
    ...
    if dataset_path == "Lin-Chen/ShareGPT4V":
    ...
    if start_idx > 0 or end_idx < len(dataset):
        output_file = output_file.parent / f"{output_file.stem}-{start_idx}-{end_idx}.json"
        ...
        while True:
            ...
            while len(items) < num_selected:
                try:
                    ...
                    item['image'] = load_image(item.get('link', item.get('url')))
                    if not item['image'] or item['image'].size[0] < 100 or item['image'].size[1] < 100:
                        if item['image'] and (item['image'].size[0] < 100 or item['image'].size[1] < 100):
                            print(f"Skipping {idx} for it's too small")
                        pbar.update(1)
                        idx += 1
                        continue
                    
                    image_postfix = item['image'].format or "jpg"
                    image_path = image_save_dir / f"{idx}.{image_postfix}"

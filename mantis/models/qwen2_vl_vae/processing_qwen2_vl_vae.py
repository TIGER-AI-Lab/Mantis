...
class Qwen2VLVAEProcessorKwargs(ProcessingKwargs, total=False):
    _defaults = {
        "text_kwargs": {
            "padding": False,
        },
...
class Qwen2VLVAEProcessor(ProcessorMixin):
    r"""
    ...
    image_processor_class = "Qwen2VLVAEImageProcessor"
    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2VLVAEProcessorKwargs],
    ) -> BatchFeature:
        """
        ...
        output_kwargs = self._merge_kwargs(
            Qwen2VLVAEProcessorKwargs,
            tokenizer_init_kwargs=self.tokenizer.init_kwargs,
            **kwargs,
        )
        if images is not None:
            image_inputs = self.image_processor(images=images, videos=None, **output_kwargs["images_kwargs"])
            image_vae_tokens = image_inputs.pop("num_vae_tokens", None)
        else:
            image_inputs = {}

        if videos is not None:
            videos_inputs = self.image_processor(images=None, videos=videos, **output_kwargs["videos_kwargs"])
            video_vae_tokens = videos_inputs.pop("num_vae_tokens", None)
        else:
            videos_inputs = {}
            ...
            for i in range(len(text)):
                while "<|image_pad|>" in text[i]:
                    text[i] = text[i].replace(
                        "<|image_pad|>", "<|placeholder|>" * image_vae_tokens[index], 1
                    )
                    index += 1
        ...
        if videos_inputs:
            ...
            for i in range(len(text)):
                while "<|video_pad|>" in text[i]:
                    text[i] = text[i].replace(
                        "<|video_pad|>", "<|placeholder|>" * video_vae_tokens[index], 1
                    )
                    index += 1

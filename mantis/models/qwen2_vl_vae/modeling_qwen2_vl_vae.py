...
class Qwen2VLVAEPreTrainedModel(PreTrainedModel):
    config_class = Qwen2VLVAEConfig
...
QWEN2_VL_VAE_INPUTS_DOCSTRING = r"""
    Args:
...
class Qwen2VLVAEForConditionalGeneration(Qwen2VLVAEPreTrainedModel, GenerationMixin):
    config_class = Qwen2VLVAEConfig
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.vae_visual = Qwen2VLVisionVAEPretrainedModel(config.vision_config)
    ...
    @add_start_docstrings_to_model_forward(QWEN2_VL_VAE_INPUTS_DOCSTRING)
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                for i in range(len(pixel_values_videos)):
                    ...
                    all_video_vae_embeds.append(video_embeds)
                
                video_embeds = torch.cat(all_video_vae_embeds, dim=1)
                video_mask = (input_ids == self.config.video_token_id).unsqueeze(-1).expand_as(inputs_embeds)
                video_embeds = video_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
                inputs_embeds = inputs_embeds.masked_scatter(video_mask, video_embeds)
                    
                # # old
                # video_embeds_lens = [video_embeds.shape[1] for video_embeds in all_video_vae_embeds]
                # video_mask = (input_ids == self.config.video_token_id)
                # assert video_mask.sum() == len(all_video_vae_embeds)
                # embed_parts = []
                # video_token_id_positions = torch.where(video_mask)[0]
                # last_position = 0
                # assert len(video_token_id_positions) == len(all_video_vae_embeds)
                # for i, video_embeds in enumerate(all_video_vae_embeds):
                #     position = video_token_id_positions[i]
                #     embed_parts.append(inputs_embeds[:, last_position:position])
                #     embed_parts.append(video_embeds)
                #     last_position = position + 1
                # embed_parts.append(inputs_embeds[:, last_position:])

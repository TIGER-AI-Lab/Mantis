...
try:
    ...
    fused_layer_norm_available = True
    print("Fused layer norm available")
except ImportError:
    fused_layer_norm_available = False
    print("Fused layer norm not available")

try:
    from flash_attn.ops.fused_dense import fused_mlp_func
    fused_mlp_available = True
    print("Fused MLP available")
except ImportError:
    fused_mlp_available = False
    print("Fused MLP not available")

try:
    from flash_attn.layers.rotary import apply_rotary_emb as fused_apply_rotary_emb
    fused_apply_rotary_emb_available = True
    print("Fused apply rotary emb available")
except ImportError:
    fused_apply_rotary_emb_available = False
    print("Fused apply rotary emb not available")
...
class PersimmonFlashAttention2(PersimmonAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.flash_attention_dropout = self.config.attention_dropout
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not fused_apply_rotary_emb_available:
        ...
        if fused_apply_rotary_emb_available:
            query_states = fused_apply_rotary_emb(query_states, cos, sin, False, True)
            key_states = fused_apply_rotary_emb(key_states, cos, sin, False, True)
        else:
            # Partial rotary embedding
            query_rot, query_pass = (
                query_states[..., : self.rotary_emb.dim],
        ...
        attn_dropout = self.flash_attention_dropout if self.training else 0.0
...
class PersimmonDecoderLayer(nn.Module):
    def __init__(self, config: PersimmonConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        # config._attn_implementation = "flash_attention_2" if is_flash_attn_2_available() else "eager" # temporary
        self.self_attn = Persimmon_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx=layer_idx)
